...
# ============================================================
# 5. NEON / COLORFUL TEXTURE HELPERS
# ============================================================
...
def add_linen_lines(img, color):
    # repurposed: subtle scanlines for neon aesthetic
    ...
    for y in range(0, H, 6):
        ...
        draw.line([(0, y), (W, y)],
                  fill=(min(255, r + v), min(255, g + v), min(255, b + v)), width=1)
    return img
...
def add_vignette(img):
    ...
    vign = 1.0 - np.clip((dist - 0.3) / 0.5, 0, 1) * 0.55
    for ch in range(3):
        arr[:, :, ch] *= vign
    return Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8))

def add_neon_grid(img, glow_color, opacity=28):
    """Subtle grid lines for cyber/neon look."""
    overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    r, g, b = glow_color
    for x in range(0, W, 90):
        d.line([(x, 0), (x, H)], fill=(r, g, b, opacity), width=1)
    for y in range(0, H, 90):
        d.line([(0, y), (W, y)], fill=(r, g, b, opacity), width=1)
    base = img.convert("RGBA")
    merged = Image.alpha_composite(base, overlay)
    return merged.convert("RGB")

def add_glow_border(img, border_color, glow_color, padding=36):
    """Neon glowing border instead of plain paper border."""
    draw = ImageDraw.Draw(img)
    r, g, b = glow_color
    # outer soft glow layers
    for offset, alpha_div in [(6, 5), (4, 4), (2, 3)]:
        p = padding - offset
        br, bg2, bb = min(255, r + 20), min(255, g + 20), min(255, b + 20)
        draw.rectangle([p, p, W - p, H - p],
                       outline=(br, bg2, bb), width=1)
    # main border
    draw.rectangle([padding, padding, W - padding, H - padding],
                   outline=border_color, width=3)
    # inner thin line
    draw.rectangle([padding + 12, padding + 12, W - padding - 12, H - padding - 12],
                   outline=border_color, width=1)

def add_decorative_border(draw, color, padding=38):
    """Kept for compatibility — delegates to draw-based neon border."""
    add_glow_border(draw._image, color, color, padding)
...
def render_reel_image(topic, secrets, theme_name, heading=None):
    ...
    if T["style"] == "neon":
        img = add_neon_grid(img, T.get("glow", T["border"]), opacity=22)
    if T.get("vignette"):
        img = add_vignette(img)

    add_glow_border(img, T["border"], T.get("glow", T["border"]), padding=36)
    draw = ImageDraw.Draw(img)
    ...
    # ── Top divider (neon style) ──
    accent = T.get("accent", T["divider"])
    y += DIVIDER_PAD // 2
    draw.line([(MARGIN, y), (W - MARGIN, y)], fill=T["divider"], width=2)
    draw.line([(MARGIN + 2, y + 4), (W - MARGIN - 2, y + 4)], fill=accent, width=1)
    mid = W // 2
    draw.polygon([(mid - 10, y + 2), (mid, y - 10), (mid + 10, y + 2), (mid, y + 14)], fill=accent)
    y += DIVIDER_PAD // 2 + 6
...
def main():
    ...
    # ── Cleanup: delete output files to save disk space ──
